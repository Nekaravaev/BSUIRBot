...
$debugToken = $config->vk->debugToken;
...
if ($request->type == 'wall_post_new') {
    ...
    foreach ($usersArray as $userUpdateId) {
        $bot->forwardWallPost($userUpdateId, 'Новая запись на стене', 'wall-'.$groupId.'_'.$postId);
    }

    $debugBot->forwardWallPost('2000000003', 'Новая запись на стене', 'wall-'.$groupId.'_'.$postId);
}
